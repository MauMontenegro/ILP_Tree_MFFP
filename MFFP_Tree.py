...
path = os.walk(Path.cwd() / 'Instances')
...
for root, directories, files in path:
    ...
    for directory in directories:
        ...
        instance = utils.generateInstance(load, str(directory))
        T = instance[0]
        N = instance[1]
        ...
        solver = pl.GUROBI_CMD(options=[("Method",1),("NodefileStart",0.5),("Threads",12)])
        prob.solve(solver)

        print("Solution Time")
        print(prob.solutionTime)
        times.append(prob.solutionTime)
...
times=[0.13022685050964355, 1.7927758693695068, 52.99954056739807, 51.94762182235718, 116.6163580417633, 219.66380834579468]
fig,ax= plt.subplots(figsize=(12,6))
ax.plot(n_nodes,times)
plt.show()